...
def classify(fingers, num_hands):
    ...
    all_f = [[int(b) for b in fh] for fh in fingers]

    # Two-hand (or more) gestures first, and check EVERY required hand's
    # pattern — not just hand[0] — otherwise a two-hand word can never
    # be distinguished from a one-hand word sharing the same pattern.
    for word, pat, minh in GTABLE:
        if minh >= 2 and num_hands >= minh and all(h == pat for h in all_f[:minh]):
            return word

    # Single-hand exact match
    for word, pat, minh in GTABLE:
        if minh == 1 and f == pat:
            return word

    # Fuzzy fallback (single-hand only, 1-finger tolerance)
    best, bd = None, 999
    for word, pat, minh in GTABLE:
        if minh > 1: continue
